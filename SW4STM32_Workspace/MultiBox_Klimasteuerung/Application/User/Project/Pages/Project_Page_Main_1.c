...
uint8_t SollVorgabe1 = 20;
uint8_t SollVorgabe2 = 20;
...
void _cbDialog_Main1(WM_MESSAGE * pMsg) {
	...
	case WM_INIT_DIALOG:
		...
		//
		// Initialization of 'SollVorgabe1 [°C]:'
		//
		hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_TC2);
		TEXT_SetFont(hItem, GUI_FONT_16_1);

		hItem = WM_GetDialogItem(pMsg->hWin, ID_SPINBOX_TCSEL);
		SollVorgabe1 = TC_Sollwert.Value.un;
		temperatureControl_setSoll(SollVorgabe1);
		SPINBOX_SetValue(hItem, SollVorgabe1);
...
//				MessStelle = DROPDOWN_GetSel(hItem);
			...
			case ID_SPINBOX_TCSEL: // Notifications sent by 'Spinbox'
				...
				case WM_NOTIFICATION_VALUE_CHANGED:
					...
					hItem = WM_GetDialogItem(pMsg->hWin, ID_SPINBOX_TCSEL);
					SollVorgabe1 = SPINBOX_GetValue(hItem);
					ParameterWrite((&TC_Sollwert), Value, SollVorgabe1)
					;
					ParameterChange_Handled = 0;
					break;
				}
...
// Update of 'Soll-Vorgabe' SpinBox
	...
	if (lastTimeSwitch_SetTemperature != timeSwitch_SetTemperature) {
		...
		SollVorgabe1 = timeSwitch_SetTemperature;
		SPINBOX_SetValue(hItem, SollVorgabe1);
		ParameterWrite((&TC_Sollwert), Value, SollVorgabe1);
		ParameterChange_Handled = 0;
	}
	...
	temperatureControl_setSoll((float) SollVorgabe1);
	SPINBOX_SetValue(hItem, SollVorgabe1);
