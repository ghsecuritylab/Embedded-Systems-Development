...
void Firmware_ProcessLogic_ReadInputs() {
    ...
    if (!LOCK_OneWire) {
        ...
        if ((Counter+0)%1000 == 0){
            ...
            DS1820_askTemp(ONEWIRE_ROM_TEMP1->Value.ROM);
            DS1820_askTemp(ONEWIRE_ROM_TEMP2->Value.ROM);
            DS1820_askTemp(ONEWIRE_ROM_TEMP3->Value.ROM);
            DS1820_askTemp(ONEWIRE_ROM_TEMP4->Value.ROM);
            DS1820_askTemp(ONEWIRE_ROM_TEMP5->Value.ROM);
        ...
        if ((Counter+800)%1000 == 0) {
            ...
            Temp = *DS18x20_readTemp(ONEWIRE_ROM_TEMP1.Value.ROM);
            Temp = *DS18x20_readTemp(ONEWIRE_ROM_TEMP2.Value.ROM);
            Temp = *DS18x20_readTemp(ONEWIRE_ROM_TEMP3.Value.ROM);
            Temp = *DS18x20_readTemp(ONEWIRE_ROM_TEMP4.Value.ROM);
            Temp = *DS18x20_readTemp(ONEWIRE_ROM_TEMP5.Value.ROM);
            RingMemory_Save(&TemperaturGehaeuse,&Temp);
            RingMemory_Save(&ONEWIRE_ROM_TEMP1,&Temp);
            RingMemory_Save(&ONEWIRE_ROM_TEMP2,&Temp);
            RingMemory_Save(&ONEWIRE_ROM_TEMP3,&Temp);
            RingMemory_Save(&ONEWIRE_ROM_TEMP4,&Temp);
            RingMemory_Save(&ONEWIRE_ROM_TEMP5,&Temp);
        }
    }
