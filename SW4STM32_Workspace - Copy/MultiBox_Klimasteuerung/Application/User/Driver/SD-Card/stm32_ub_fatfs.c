...
void SD_Test() {
	FATFS_t result;
	uint32_t fileSize0 = 0, fileSize1 = 0;

    SD_LOCK = 1;

    result = UB_Fatfs_Mount(MMC_1);
	if (result != FATFS_OK) {
	...
	result = UB_Fatfs_OpenFile(&FileObj, "0:/Test.txt", F_WR_NEW);
	if (result != FATFS_OK) {
		Firmware_ProcessLogic_ErrorHandler(0);
	}
	...
	fileSize0 = UB_Fatfs_FileSize(&FileObj);

	SD_LOCK = 0;

	SD_LOCK = 1;

	result = UB_Fatfs_WriteString(&FileObj, "Test Eintrag Zeile 1");
	if (result != FATFS_OK) {
		Firmware_ProcessLogic_ErrorHandler(0);
	}
	...
	fileSize1 = UB_Fatfs_FileSize(&FileObj) - fileSize0;
